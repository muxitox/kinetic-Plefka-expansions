...
class HiddenIsing:  # Asymmetric Ising model with hidden activity simulation class
    ...
    def coordinated_gradient_descent(self, dLdH, dLdJ, dLdK, dLdL, dLdb_0, dLdh_0, eta):
        ...
        max_max = np.argmax(np.abs([max_H, max_J, max_K, max_L, max_b0, max_h0]))
        if max_max == 0:
        ...
        if max_max == 1:
            ...
            self.h_0[max_b0_idx] = self.h_0[max_h0_idx] + eta * max_h0
            error = np.abs(max_h0)
    ...
    def fit(self, s, eta, max_reps, T_ori, T_sim, original_moments, gradient_mode='regular'):
        ...
        while error > error_lim and rep < max_reps:
            ...
            dLdh_0 = np.zeros(self.visible_size)
